...
A reader might expect the find-QR to live where the setup QR does — operator side, shown from the admin team/agent list (in a modal, see ADR 0024). It deliberately does not: a [Find](../../CONTEXT.md) happens during play, in the moment a team reaches Mister X, so the QR is presented live by the hunted agent on the device already in their hand.
